...
class DataProcessor:
    ...
    This class provides methods for persisting scraped data to the database and
    cleaning up old records. (Storage was migrated from CSV/JSON files to a
    database; the scraping logic itself is unchanged.)
    """

    @staticmethod
    def save_to_csv(data, filename=None):
        """
        Persist cryptocurrency price data to the database.

        The method name and signature are retained for backwards compatibility;
        data is now written to the `crypto_prices` table instead of a CSV file.

        Args:
            data (list): List of dictionaries containing cryptocurrency data
            filename: Unused; retained for backwards compatibility.
        """
        db.save_prices(data)

    @staticmethod
    def save_top_gainers_data(data, filename=None):
        """
        Persist top gainers data to the database.

        Each call inserts a new top-gainers snapshot (the full list) with a
        creation timestamp, replacing the previous main-file + timestamped-backup
        JSON scheme.
    ...
    def cleanup_old_top_gainers_data(max_age_hours=24):
        """
        Clean up old top gainers snapshots from the database.

        Args:
            max_age_hours (int, optional): Maximum age of snapshots to keep, in hours. Defaults to 24.
        """
        cutoff_time = datetime.now() - timedelta(hours=max_age_hours)
        db.delete_top_gainers_before(cutoff_time)
